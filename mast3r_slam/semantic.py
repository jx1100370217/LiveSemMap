...
除 POI 标签外, 每帧同时判读"局部空间结构"(space_kind/at_transition/passages/objects
等字段, 见 ANNOTATION_SCHEMA), 供房间级拓扑建图(RoomTopoBuilder)在线切分房间段。

- 输入为该帧的前视高清图(_0) + 4 张环视鱼眼(_1.._4 ≈ 前/右/后/左)拼成的带方位标签
  2x2 拼图, 由前处理放在 datasets/<name>/surround/{frame_id:06d}_{0..4}.jpg,
  按 frame_id 从磁盘读取直接发 VLM;
...
# 空间形态: 房间拓扑建图的切分信号 (与 category 解耦: category 是"这里有什么 POI",
# space_kind 是"机器人身处什么形态的空间" —— 走廊里路过公司门口时
# category=doorway 而 space_kind=corridor)
SPACE_KINDS = ("room", "corridor", "junction", "open_area", "doorway")
PASSAGE_DIRS = ("front", "right", "back", "left")
PASSAGE_KINDS = ("door", "opening", "corridor", "stairs", "elevator")
...
ANNOTATION_SCHEMA = {
    ...
    "properties": {
        ...
        "space_kind": {"type": "string", "enum": list(SPACE_KINDS)},
        "at_transition": {"type": "boolean"},
        "transition_conf": {"type": "number"},
        "passages": {"type": "array", "maxItems": 6, "items": {
            "type": "object",
            "properties": {
                "dir": {"type": "string", "enum": list(PASSAGE_DIRS)},
                "kind": {"type": "string", "enum": list(PASSAGE_KINDS)},
                "signage": {"type": "string", "maxLength": 24},
                "leads_to": {"type": "string", "maxLength": 16},
            },
            "required": ["dir", "kind", "signage", "leads_to"],
        }},
        "objects": {"type": "array", "maxItems": 10,
                    "items": {"type": "string", "maxLength": 16}},
    },
    "required": ["category", "name", "description", "confidence", "landmark",
                 "signage", "space_kind", "at_transition", "transition_conf",
                 "passages", "objects"],
}

SURROUND_VIEWS = ("前", "右", "后", "左")  # camera_1..4 的大致朝向
# 2x2 拼图各格标签 (烧进图中, 供 VLM 判 passage 方位): 左上/右上/左下/右下
_GRID_LABELS = ("前 FRONT", "右 RIGHT", "后 BACK", "左 LEFT")
_CJK_FONTS = ("/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc",
              "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
              "/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf")

PROMPT = """你是室内导航语义标注员。给你巡检机器人同一时刻的两张图：第 1 张是前视高清相机画面（分辨率最高，文字最清楚）；第 2 张是 4 路环视鱼眼的 2×2 拼图，每格上方已标注朝向——左上=前FRONT、右上=右RIGHT、左下=后BACK、右下=左LEFT（画面中可能拍到机器人自身部件和跟随的操作人员，忽略它们）。请综合两张图完成两项任务：A. 为当前位置打一个对导航最有用的 POI 语义标签；B. 判读机器人当前所处空间的结构。

【任务A：POI 标签】类别(category)必须从以下列表中选一个：
- doorway: 门口/门附近
...
【任务B：空间结构判读】
- space_kind：机器人当前**身处**的空间形态（注意与 category 区分——走廊里路过某公司门口时 category=doorway 但 space_kind=corridor）：
  room=封闭房间内（四周多为墙/家具围合）；corridor=走廊/过道中；junction=走廊交汇口/转角；open_area=开阔区域（大厅/开放办公区）；doorway=正处在门口/门框/闸机处。
- at_transition：机器人是否**正在穿越**出入口（门框/门槛/闸机就在身旁或正跨越，即将从一个空间进入另一个空间）。远处看到门不算，必须正在通过或紧贴门口。
- transition_conf：对 at_transition 判断的把握，0.0~1.0。
- passages：画面中清晰可见的**可通行出入口**，逐个列出（最多 6 个，被挡住/太远的不列；走廊本身的延伸方向也算，kind=corridor）：
  dir=该出入口相对机器人的方位（front/right/back/left，按拼图各格标注的朝向判断；第 1 张前视图对应 front）；
  kind：door=有门框的门，opening=无门的开口/门洞，corridor=走廊延伸方向，stairs=楼梯口，elevator=电梯门；
  signage=该出入口处可读的专属文字（门牌/公司名），读不到给 ""；
  leads_to=透过该出入口能看到什么（如"办公区"/"走廊"），看不清给 ""。
- objects：画面中有助判断空间功能的家具/设备，中文短词（如 沙发/前台/工位/打印机/冰箱/闸机/绿植墙），最多 10 个，没有给 []。

只输出一个 JSON 对象：
{"category": "类别", "name": "节点名称(优先用画面专属文字, 2-12字)", "description": "一句话中文描述(地标在哪个方位、有什么特征)", "confidence": 0.0到1.0, "landmark": true或false, "signage": ["画面中清楚读出的专属名称文字"], "space_kind": "空间形态", "at_transition": true或false, "transition_conf": 0.0到1.0, "passages": [{"dir": "方位", "kind": "类型", "signage": "文字或空串", "leads_to": "通往或空串"}], "objects": ["家具设备"]}
...
def _surround_grid(fisheye_paths):
    """把 4 张环视鱼眼拼成带方位标签的 2x2 拼图, 返回 JPEG bytes (无可用图返回 None)。

    每格缩半(1024x819 -> 512x410, 单格视觉 token 量与旧 5 图方案的单张鱼眼一致)并在
    顶部条烧方位标签(前/右/后/左), 使 VLM 在单张图的注意力场内判读 passage 方位;
    个别缺失的格填黑。fisheye_paths: {1..4: Path}, 键为 camera 序号(1前/2右/3后/4左)。
    """
    import io

    from PIL import Image, ImageDraw, ImageFont

    if not fisheye_paths:
        return None
    cw, ch, strip = 512, 410, 34
    font = None
    for fp in _CJK_FONTS:
        try:
            ...
            continue
    grid = Image.new("RGB", (cw * 2, (ch + strip) * 2), (16, 16, 16))
    draw = ImageDraw.Draw(grid)
    for i in range(4):  # 0前/1右/2后/3左 -> 左上/右上/左下/右下 (与 PROMPT 描述一致)
        x, y = (i % 2) * cw, (i // 2) * (ch + strip)
        p = fisheye_paths.get(i + 1)
        if p is not None:
            im = Image.open(p).convert("RGB").resize((cw, ch), Image.BILINEAR)
            grid.paste(im, (x, y + strip))
        label = _GRID_LABELS[i] + ("" if p is not None else " (缺失)")
...
class SemanticAnnotator:
    ...
    def __init__(self, api_url, shared_ann, surround_dir,
        ...
        self.inflight = set()          # 已入队未完成的 kf_idx (供按序消费判界)
        self.fail_streak = 0
        self.disabled = False
        self.threads = [
    ...
    def submit(self, kf_idx, frame_id):
        ...
        self.inflight.add(kf_idx)
        self.q.put((kf_idx, int(frame_id)))

    def min_inflight(self):
        """当前已入队未完成的最小 kf_idx (无在途时返回极大值)。房间拓扑等按序
        消费者用: 仅序号小于它的标注保证不会再有更早的乱序补全。"""
        return min(tuple(self.inflight), default=1 << 30)
    ...
    def _annotate(self, img_paths, retries=2):
        """img_paths: 该帧的语义图路径列表 (_0 前视高清 + _1.._4 环视鱼眼, 允许缺个别)。
        前视保持原图(读门牌/公司名), 4 鱼眼拼成带方位标签的 2x2 拼图, 共发 2 张图。"""
        import requests

        if not img_paths:
            return None
        front = next((p for p in img_paths if p.stem.endswith("_0")), None)
        fisheyes = {int(p.stem[-1]): p
                    for p in img_paths if not p.stem.endswith("_0")}
        content = []
        if front is not None:
            b64 = base64.b64encode(front.read_bytes()).decode()
            content.append({"type": "image_url",
                            "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
        grid = _surround_grid(fisheyes)
        if grid is not None:
            b64 = base64.b64encode(grid).decode()
            content.append({"type": "image_url",
                            "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
        if not content:
            return None
        ...
        payload = {
            ...
            "max_tokens": 600,  # passages/objects 数组比旧 schema 长, 300 会截断
            "temperature": 0.0,
        ...
        for _ in range(retries + 1):
            try:
                ...
                # 空间结构字段规范化 (guided json 保证枚举合法, 此处兜底缺省/异常)
                if ann.get("space_kind") not in SPACE_KINDS:
                    ann["space_kind"] = "corridor"
                ann["at_transition"] = bool(ann.get("at_transition"))
                ann["transition_conf"] = float(
                    np.clip(ann.get("transition_conf", 0.0), 0.0, 1.0))
                ann["passages"] = [
                    {"dir": p["dir"], "kind": p["kind"],
                     "signage": (p.get("signage") or "").strip(),
                     "leads_to": (p.get("leads_to") or "").strip()}
                    for p in (ann.get("passages") or [])
                    if isinstance(p, dict) and p.get("dir") in PASSAGE_DIRS
                    and p.get("kind") in PASSAGE_KINDS]
                ann["objects"] = [o.strip() for o in (ann.get("objects") or [])
