...
class SemanticAnnotator:
    ...
    def __init__(self, api_url, shared_ann, model="qwen3.5-35b-a3b", workers=8, timeout=30):
        # workers=8 配合 vLLM --max-num-seqs 8: 实测吞吐 ~5.5 帧/s (3 并发仅 ~2),
        # 高于 insight9 的关键帧生成率(~2.8/s), 标注可实时跟上建图。
        # 曾评估换 Qwen3.5-2B 提速: 类别一致率 76% 尚可, 但 landmark 判断全失效
        # (49/49 全 true)、地标类别一致率仅 4/9, 节点质量不达标, 故保持 9B 提并发。
